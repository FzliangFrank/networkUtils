...
This happens to me when a restyle visNetworkOutput id from `plot` to `visNetworkId`
...
server <- function(input, output, session) {
  ...
  output$text <- renderPrint({
    ...
    print(glue::glue(
      "the difference between c (a reactive expression) and 
      a g (a reactive value) is x is type of {mode(x)}, 
      a type of {mode(g$a)}"
    ))
  })
  observe({
